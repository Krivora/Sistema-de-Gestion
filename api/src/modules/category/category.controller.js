...
export async function createCategory(req, res, next) {
  try {
    ...
    res.status(201).json(await CategoryService.addCategory({ name, description, code }, req.user.client_id, req.user.role_name));
  } catch (err) { next(err); }
}
...
export async function updateCategory(req, res, next) {
  try {
    ...
    if (!name) return res.status(400).json({ error: "El nombre es requerido" });
    res.json(await CategoryService.editCategory(req.params.id, req.user.client_id, req.user.role_name, { name, description }));
  } catch (err) { next(err); }
}

export async function activateCategory(req, res, next) {
  try {
    const cat = await CategoryService.activateCategory(req.params.id, req.user.client_id, req.user.role_name);
...
export async function deactivateCategory(req, res, next) {
  try {
    const cat = await CategoryService.deactivateCategory(req.params.id, req.user.client_id, req.user.role_name);
...
export async function deleteCategory(req, res, next) {
  try {
    const cat = await CategoryService.deleteCategory(req.params.id, req.user.client_id, req.user.role_name);
