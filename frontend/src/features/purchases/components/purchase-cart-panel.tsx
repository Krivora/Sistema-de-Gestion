...
import { Search, Plus, Trash2, Package, ShoppingCart } from "lucide-react"
...
interface Props {
    ...
    onUpdateCart: (id: number, field: "qty" | "unit_cost", v: string) => void
            ...
            {cart.length === 0 ? (
                ...
                <div className="space-y-1">
                    <div className="grid grid-cols-[1fr_100px_120px_36px] gap-3 px-3 py-1.5 text-xs font-medium text-muted-foreground uppercase tracking-wide">
                        <span>Producto</span><span className="text-center">Cantidad</span><span className="text-center">Costo unit.</span><span />
                    </div>
                    {cart.map((item) => (
                        <div key={item.product_id} className="grid grid-cols-[1fr_100px_120px_36px] gap-3 items-center px-3 py-2 rounded-lg hover:bg-muted/40 transition-colors">
                            <div className="flex items-center gap-2.5 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant="secondary" className="text-[10px] px-1 py-0 h-4">Stock: {item.current_stock}</Badge>
                                    </div>
                                </div>
                            </div>
                            <Input value={item.qty} onChange={(e) => onUpdateCart(item.product_id, "qty", e.target.value)} inputMode="numeric" className={cn("h-8 text-center text-sm", (!item.qty || Number(item.qty) <= 0) && "border-destructive")} />
                            <Input value={item.unit_cost} onChange={(e) => onUpdateCart(item.product_id, "unit_cost", e.target.value)} inputMode="decimal" className={cn("h-8 text-right text-sm", item.unit_cost === "" && "border-destructive")} />
