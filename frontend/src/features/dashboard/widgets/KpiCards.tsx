            ...
            <div className="stat-body">
                ...
                {sub && !loading && (
                    <span className={`stat-sub ${trend === "up" ? "sub-up" : trend === "down" ? "sub-down" : ""}`}>
                        {trend === "up" && <ArrowUpRight size={11} aria-hidden />}
                        {trend === "down" && <ArrowDownRight size={11} aria-hidden />}
                        {sub}
                    </span>
                )}
            </div>
    ...
    return (
        <div className="stats-grid">
            ...
            <StatCard
                ...
                icon={TrendingDown} color="#e05c5c" loading={loading}
            ...
            <StatCard
                label="Margen neto"
                icon={BarChart3} color={BLUE} loading={loading}
                value={summary ? `${metrics.marginFormatted}%` : "—"}
                sub={summary ? formatCurrency(metrics.profit) : undefined}
                trend={metrics.margin > 30 ? "up" : metrics.margin > 0 ? "neutral" : "down"}
            />
            <StatCard
                label="Alertas stock"
                icon={AlertTriangle} color={ACCENT} loading={loading}
