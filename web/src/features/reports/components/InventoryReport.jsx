...
import { ArrowDownward, ArrowUpward, IosShare } from "@mui/icons-material";
...
export default function InventoryReport() {
  ...
  const { branches } = useBranches(); // lista de sucursales
  ...
  useEffect(() => {
    fetchStock(branchId || null, categoryId || null);
  }, [branchId, categoryId]);
  ...
  const filteredData = useMemo(() => {
    ...
    let result = data.stock.filter((r) => {
      const text = `${r.branch_name} ${r.product_name} ${r.category_name || ""} ${r.sku
        }`.toLowerCase();
  ...
  const tableContainer = `rounded-xl border shadow-sm overflow-x-auto ${darkMode ? "border-gray-700 bg-[#1a1a1a]" : "border-gray-200 bg-white"
    }`;
  ...
  return (
    <div className={tableContainer}>
      ...
      <div className="flex flex-wrap justify-between items-center p-4 border-b border-gray-600/30">
        <h3
          className={`font-semibold text-lg ${darkMode ? "text-gray-100" : "text-gray-800"
            }`}
        >
          Inventario general
        </h3>
      ...
      <div className="p-4 border-b border-gray-600/30 flex gap-4 flex-wrap">
        {/* Sucursal */}
        <FormControl
          size="small"
          sx={{
        ...
        <thead>
          <tr>
            {[
              ...
              <th
                key={col.key}
                className={`${thStyle} ${col.align === "right" ? "text-right pr-6" : "text-left"
                  }`}
        ...
        >
          {loading ? (
            ...
            <tr>
              <td
                colSpan={5}
                className={`text-center py-6 ${darkMode ? "text-gray-500" : "text-gray-600"
                  }`}
              >
            ...
            paginatedData.map((r, idx) => (
              <tr
                key={idx}
                className={`transition-colors ${darkMode ? "hover:bg-[#2a2a2a]" : "hover:bg-gray-50"
                  }`}
              >
                ...
                <td
                  className={`${tdStyle} text-start ${darkMode ? "text-gray-200" : "text-gray-800"
                    }`}
                >
                  ${r.price ? parseFloat(r.price).toFixed(2) : "0.00"}
                </td>
                <td
                  className={`${tdStyle} text-start font-medium ${r.stock <= 3
                    ? "text-red-500"
                    : darkMode
                      ? "text-gray-100"
                      : "text-gray-800"
